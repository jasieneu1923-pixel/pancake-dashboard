...
# A. Đơn hàng: Chỉ lấy từ ngày 24/04/2026
all_orders_raw = []
for p in range(1, 6): # Lấy tối đa 5 trang đơn hàng gần nhất
    batch = fetch_pancake_data("orders", {
        "limit": 100, 
        "mode": "all", 
        "page": p,
        "inserted_since": "2026-04-24T00:00:00Z" # Lọc từ 24/04/2026
    })
    data = batch.get("data", [])
    if not data: break
    all_orders_raw.extend(data)

# B. Nhập hàng: Lấy toàn bộ (không lọc ngày)
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "📑 Đơn hàng (Từ 24/04)", "📦 SP trong đơn", "📥 Toàn bộ Nhập hàng", "📊 Thống kê Biến thể", "📈 Thống kê Sản phẩm"
])
